const AddTaskForm = ({addNewTask, newTaskTitle, setNewTaskTitle, ref}) =>{
    return(
        ...
        <div className="flex">
            ...
            <input 
            className="bg-white h-30 rounded pl-3 resize-none pb-20" 
            placeholder="Your text" type="text" 
            value={newTaskTitle}
            onInput={(event) => setNewTaskTitle(event.target.value)}/>
            <button onClick={addNewTask} type="submit" className="bg-gray-700 text-white rounded-3xl h-10 w-15 flex-end mt-3">Add</button>
            </form>
        </div>
        </div>
    )
